...
Yet another tedious and boring problem.  

The solution is quite straight forward, but it has some gotchas.

For example, in the sample `After 1 cycle` the first row is deleted because it was left empty, but there is no warning or clarification.

Another gotcha (that's on me) is assuming that you don't need to set anything if don't see any change.

Finally, the space required for the first and second solution isn't that big, but still very slow for the 4d space.

My solution is slow, so it will take almost 1min for part 2.