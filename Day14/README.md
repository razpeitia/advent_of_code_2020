...
There are some tricky parts for example the first examples are a little misleading, because `11` and `101` are actual decimals and not binary numbers. So, fast readers beware.
Another tricky part the bit shift, depending on how you store and access your mask it can be tricky.
Finally, generating all addresses in the second part can be done easily with a single vector, you just have to be a bit clever.