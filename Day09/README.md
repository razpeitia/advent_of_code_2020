...
I went with the brute force / naive approach and for some reason it was very slow without compiling it with the `--release` flag.
I think the only tricky part is that it uses large numbers so, they may not fit in 32 bits numbers, specially when you add them together.