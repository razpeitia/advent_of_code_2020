...
Finally, something more meaty.

The first part it can be done with `union-find` but I haven't implemented that myself, so, I went with the slow solution of for each node that is not `shiny gold` check if I can reach `shiny gold`.

For the second part is actually easier than the first one, all you have to do is count the bags in `shiny node` and then for each bag type count the bags recursively and multiply that by the number of bags needed according to that type. 